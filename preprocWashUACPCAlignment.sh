...
echo " --> Concatenating matrices to get full FOV to MNI"
...
echo " --> Geting a 6 DOF approximation"
${FSLDIR}/bin/aff2rigid "$WD"/full2std.mat "$WD/$OutputMatrix"
# the error is here
echo "Output matrix: $OutputMatrix"

# Create a resampled image (ACPC aligned) using spline interpolation
echo " --> Creating a resampled image"
${FSLDIR}/bin/applywarp --rel --interp=spline -i "$Input" -r "$Reference" -o "$Output" --premat="$WD/$OutputMatrix" 
