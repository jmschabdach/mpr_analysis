...
class Subj:
    ...
    def __init__(self, subjpath):
        ...
        self.subj = subjpath.split("/")[-1]
        self.session = subjpath.split("/")[-1]+"_sess" #subjpath.split("/")[-2]
        self.run = subjpath.split("/")[-1]+"_run" #subjpath.split("/")[-1]

        # blank session ID
        self.sess_id = None

    def get_subject_path():
        return self.subjpath
    ...
    def write_to_results(self, file_name, side_lr, column_names, conn):
        ...
        if side_lr:
            file_name_path = os.path.join(self.subjpath, 
    ...
    def write_measures(self, conn):
        ...
        '''
        aseg_path = os.path.join(self.subjpath, "aseg.stats")
...
def main():
    ...
    for subj_dir in os.listdir(path):         
        print(subj_dir)
        fs_session = Subj(os.path.join(path, subj_dir))
